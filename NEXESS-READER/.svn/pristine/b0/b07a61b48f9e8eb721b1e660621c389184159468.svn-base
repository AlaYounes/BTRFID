...
namespace fr.nexess.hao.weight.device.pcb12 {
    ...
    public class Pcb12Handler : WeightReader, ComListener {
        ...
        protected SerialComHandler getPcb12ComHandler(string port) {
            ...
            try
            {
                ManObjSearch = new ManagementObjectSearcher("Select DeviceId, Caption from  Win32_PnPEntity where DeviceId like '%" + HUB_MANUFACTURER_NAME + "%" + VENDOR_ID + "%" + HUB_PRODUCT_ID + "%' ");

                foreach (ManagementObject ManObj in ManObjSearch.Get())
                {
                    string parentName = ManObj["DeviceId"].ToString().Split('\\')[1].Split('+')[2];
                    ManObjSearch = new ManagementObjectSearcher("Select Location from Win32_PnPSignedDriver where DeviceId like '%" + VENDOR_ID + "&" + HUB_PRODUCT_ID + "%" + parentName.Substring(0, parentName.Length - 1) + "%' ");
                    foreach (ManagementObject ManObj2 in ManObjSearch.Get())
                    {
                        if (ManObj2["Location"].ToString().Contains("Port_#0006"))
                        {
                            port = ManObj["Caption"].ToString();
                            port = port.Substring(port.LastIndexOf("(COM")).Replace("(", string.Empty).Replace(")", string.Empty);
                        }
                    }
                }
            }
        ...
        protected void performInventoryScan() {
            ...
            try {
                foreach (var pcbAddr in PcbAddresses) {
                    ...
                    List<String> rawWeightValues = new List<string>(5); // { "0.032", "0.000", "0.000", "0.000", "0.000" };//getPcbWeightsSync(pcbAddr);
                    string myVar = "0.032";
                    rawWeightValues.Add(myVar);
